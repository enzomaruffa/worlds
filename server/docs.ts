...
import { b64, fromB64, type DocClient, type DocState } from "./doctypes";

export { b64, fromB64, type DocClient, type DocState } from "./doctypes";
...
// snapshot is written, so a subscriber arriving mid-close waits for it and then loads a fresh
// room that sees that snapshot — two rooms for one document never overlap.
...
export async function getRoom(site: string, name: string): Promise<DocRoom> {
  ...
  const loading = (async (): Promise<DocRoom> => {
...
// The registry entry, if the document is open — for callers that must not open it
// (a socket closing, say) but need to leave a room they joined.
export function peekRoom(site: string, name: string): Promise<DocRoom> | undefined {
...
// rows go and the site's quota slot frees. Whether the caller may do this is the route's decision.
