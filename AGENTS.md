...
- `server/` — Bun HTTP + WS server, one module per concern:
  ...
  - `policies.ts` `.world.json` rules (collections, uploads, docs schemas, backend) · `docs.ts`+`docschema.ts`+`docremote.ts`
    server-held Yjs documents · `proxy.ts` site backend passthrough · `cluster.ts` multi-pod links + leases
