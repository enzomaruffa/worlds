...
- **`worlds.doc(name, handlers)`** — ONE server-held CRDT document (Yjs): `onState`/`onUpdate` in, `send(update)` out; validated against the site's `.world.json` `docs` schema on commit. See reference recipe 10c.
...
**Stack:** Bun + TypeScript, one runtime dep (`yjs`, for `worlds.doc`). Postgres for state, a `BlobStore` for files.
...
- `server/` — Bun HTTP + WS server, one module per concern:
  ...
  - `docs.ts`+`docschema.ts` server-held Yjs documents · `policies.ts` the `.world.json` `docs` schemas they are checked against
