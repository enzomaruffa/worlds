...
import { scene, sun, hemi, ambient, HALF, PLOT, heightAt, setSkyColors, revealHalf } from "./world.js";
...
function initParticles() {
  ...
  for (let i = 0; i < 260; i++) {
    const R = revealHalf() * 2;
    const x = (Math.random() - 0.5) * R, z = (Math.random() - 0.5) * R, y = Math.random() * 26;
...
function updateFireflies(dt, t, night) {
  ...
  for (let i = 0; i < fmeta.length; i++) {
    ...
    const rh = revealHalf();
    if (f.x > rh) f.x = -rh; if (f.x < -rh) f.x = rh;
    if (f.z > rh) f.z = -rh; if (f.z < -rh) f.z = rh;
...
function updateWeather(dt, t) {
  ...
  else if (part === "leaves") { col = 0xd8843a; size = 0.5; fall = 0.8; sway = 1.3; count = wmeta.length * 0.4; }
  ...
  for (let i = 0; i < wmeta.length; i++) {
    ...
    const R = revealHalf() * 2;
    if (w.y < floor) { w.y = 22 + Math.random() * 6; w.x = (Math.random() - 0.5) * R; w.z = (Math.random() - 0.5) * R; }
    if (w.x > R / 2) w.x -= R; if (w.x < -R / 2) w.x += R;
